...
function checkMvDraft() {
    loadNewMvList(); // 🔥 修復：確保進入草稿時，資料清單已準備好
    ...
    if(val === 'NEW') { 
        ...
        try { 
            ...
            res.items.forEach(item => { 
                ...
                if(!existing) { 
                    allMvItems.unshift({ id: item.id, name: item.name, loc: item.loc, isMisc: item.id.startsWith('MISC-'), tempCode: item.tempCode, expectedLoc: item.expectedLoc || '待定', qty: item.qty || '1' }); 
                } else { 
                    existing.tempCode = item.tempCode; 
                } 
            }); 
            document.getElementById('newMvSelectedCount').innerText = newMvCart.size; 
            filterNewMvList(); 
