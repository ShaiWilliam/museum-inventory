    ...
    if(val === 'NEW') { document.getElementById('newMvName').value = ''; document.getElementById('newMvDesc').value = ''; clearNewMvSelection(); clearMvDraft(); } 
        ...
        try {
            ...
            res.items.forEach(item => {
                newMvCart.set(item.id, { name: item.name, loc: item.loc, isMisc: item.id.startsWith('MISC-'), tempCode: item.tempCode, expectedLoc: item.expectedLoc || '待定' });
                let existing = allMvItems.find(x => x.id === item.id);
                if(!existing) { allMvItems.unshift({ id: item.id, name: item.name, loc: item.loc, isMisc: item.id.startsWith('MISC-'), tempCode: item.tempCode, expectedLoc: item.expectedLoc || '待定' }); } else { existing.tempCode = item.tempCode; }
            });
...
function addMiscItem() {
    ...
    allMvItems.unshift({ id: miscId, name: name, loc: loc, isMisc: true, tempCode: tCode, expectedLoc: '待定' }); newMvCart.set(miscId, { name: name, loc: loc, isMisc: true, tempCode: tCode, expectedLoc: '待定' });
...
function parseImportData() {
    ...
    lines.forEach(line => {
        ...
        if (cols.length === 1) { parsedId = cols[0]; } else { 
            ...
            parsedExpectedLoc = cols.length > 3 ? cols[3] : ''; 
        }
        if (!parsedId) return;
...
async function handleLocScan(msg) {
    ...
    if (cleanMsg.startsWith("LOC:") || cleanMsg.startsWith("HANDOFF:")) {
        if (cleanMsg.startsWith("HANDOFF:")) {
            let pin = cleanMsg.substring(8);
            return processHandoff(pin);
        }
...
function resumeHandoffPrompt() {
    startLocScanner('mvLoc'); // 共用地點掃描器來掃 HANDOFF QR Code
    setTimeout(() => {
        document.getElementById('locScannerTitle').innerText = "掃描交接 QR Code";
        let manualBtn = document.createElement('button');
        manualBtn.className = "btn btn-info w-100 mt-2 fw-bold py-3 fs-5 text-white";
...
function renderTable(tid, data, cls, edit) {
    document.querySelector(`#${tid} tbody`).innerHTML = data.map(x => {
        ...
        return `<tr><td><input type="checkbox" class="${cls}" value="${x.rowIndex}"></td><td><b class="${x.qrCode.startsWith('MISC') ? 'text-danger' : 'text-dark'}">${escapeHTML(displayId)}</b>${tcBadge}<br><span class="text-primary small fw-bold">${escapeHTML(x.name)}</span><br><small class="text-muted">原: ${escapeHTML(x.oldLoc)}</small></td><td><span class="text-success fw-bold">${escapeHTML(x.newLoc)}</span>${expectedWarning}<br><small class="badge bg-light text-dark border mt-1">${escapeHTML(x.boxName||'未裝箱')}</small></td><td>${edit ? `<small class="text-muted d-block mb-1">搬運: ${escapeHTML(x.staff)}</small><button class="btn btn-sm btn-outline-primary" onclick="promptEdit(${x.rowIndex},'${safeLoc}','${safeBox}')">📝修改</button>` : `<small class="text-muted d-block mb-1">搬運: ${escapeHTML(x.staff)}</small><small class="badge bg-success">核對: ${escapeHTML(x.manager)}</small>`}</td></tr>`;
    }).join('');
}
