...
function printConditionReport() {
    ...
    let overLayer = document.getElementById('printOverlay'); if(overLayer) overLayer.style.display = 'none';
    let repLayer = document.getElementById('printReportOverlay'); if(repLayer) repLayer.style.display = 'none';
...
function closePostPrintAndReturn() {
    let modalEl = document.getElementById('condPostPrintModal');
...
async function submitVendorReport() {
    ...
    try {
        await callAPI('uploadVendorReport', payload);
        alert(`✅ 檔案上傳成功！\n紀錄已建立完成，可於歷史清單中檢視。`);
        backToCondDashboard(true);
    } catch(e) { alert("上傳失敗：" + e.message); } finally { hideMiniLoading(); }
}
